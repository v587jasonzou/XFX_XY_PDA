...
import jx.yunda.com.terminal.modules.ORGBookNew.adapter.DispatchTPListAdapterNew;
import jx.yunda.com.terminal.modules.ORGBookNew.presenter.ITPWorkNew;
import jx.yunda.com.terminal.modules.ORGBookNew.presenter.TPDispatchPresenterNew;
...
//范围活fragment
public class TpWorkNewActivity extends BaseActivity<TPDispatchPresenterNew> implements ITPWorkNew {
    ...
    DispatchTPListAdapterNew adapter;
    int EquipPosition = 0;
    int tabStatus = 0;
    ...
    @Override
    protected TPDispatchPresenterNew getPresenter() {
        return new TPDispatchPresenterNew(this);
    }
    ...
    public void initData() {
        ...
        adapter = new DispatchTPListAdapterNew(this, mList);
...
//        mPresenter.getDispatchTrains(SysInfo.userInfo.emp.getOperatorid().toString(), "");
        onTabSelectedListener = new VerticalTabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabView tab, int position) {
                ...
                if (ticketTrainBeans.size() > 0) {
                    ...
                    param.put("workPlanIDX", ticketTrainBeans.get(position).getIdx());
                    if (tabStatus == 0) {
                        ...
                        mPresenter.getTicketList(start, 1000,
                                SysInfo.userInfo.emp.getOperatorid().toString(), "true", JSON.toJSONString(param), 0);
                    }
        ...
        tbAssign.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                if (ticketTrainBeans.size() > 0) {
                    getProgressDialog().show();
                    Map<String, String> param = new HashMap<>();
                    param.put("workPlanIDX", ticketTrainBeans.get(EquipPosition).getIdx());
        ...
        adapter.SetOnBackClickListner(new DispatchTPListAdapterNew.OnBackClickLisnter() {
            @Override
            public void onBackClick(int position) {
                getProgressDialog().show();
                mPresenter.UpLevel(mList.get(position).getIdx(),"schedu","orginalGroup");
            }
        });
    }
...
//            Map<String, String> param = new HashMap<>();
//            param.put("workPlanIDX",ticketTrainBeans.get(0).getIdx());
//            mPresenter.getTicketList(start, 1000,
//                    SysInfo.userInfo.emp.getOperatorid().toString(), "false", JSON.toJSONString(param), 0);
//        }
...
//                            users.addAll(userlistTemp);
    ...
    public void onDispatchSuccess(String msg) {
        ToastUtil.toastShort(getString(R.string.success_dispatch));
        getProgressDialog().show();
        Map<String, String> param = new HashMap<>();
        param.put("workPlanIDX", ticketTrainBeans.get(EquipPosition).getIdx());
        if (tabStatus == 0) {
