...
//范围活fragment
public class ScheduleTpWorkFragment extends BaseFragment<ScheduleTPDispatchPresenter> implements ITPWork {
...
//                            users.addAll(userlistTemp);
    ...
    public void onDispatchSuccess(String msg) {
        ...
        if(tabStatus==0){
            adapter.setStatus(0);
            mPresenter.getJT28ListNew(ticketTrainBeans.get(0).getIdx(), "no");
        } else {
            adapter.setStatus(1);
            mPresenter.getJT28ListNew(ticketTrainBeans.get(0).getIdx(), "yes");
        }
    }
