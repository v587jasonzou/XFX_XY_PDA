...
public class WorkshopTaskDispatchTrainAdapter extends BaseAdapter {
    public Context mcontext;
    public List<TrainForWorkshopTaskDispatch> list;
    ViewHolder holder;
    public WorkshopTaskDispatchTrainAdapter(Context context, List<TrainForWorkshopTaskDispatch> list){
        mcontext = context;
        this.list = list;
    }
    @Override
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(list!=null&&list.size()>0){
            ...
            if (list.get(position).getFixDataFwh() > 0 || list.get(position).getFixDataTP() > 0) {
