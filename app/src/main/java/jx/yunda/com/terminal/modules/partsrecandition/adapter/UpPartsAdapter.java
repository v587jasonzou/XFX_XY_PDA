...
public class UpPartsAdapter extends BaseAdapter {
    Context mContext;
    List<ChildUpEquipBean> mList;
    ViewHolder holder;
    OnUpClickListner onUpClickListner;
    int status = 0;
    String type = "";
    public UpPartsAdapter(Context context, List<ChildUpEquipBean> list, String type) {
        mContext = context;
        mList = list;
        this.type = type;
    }
    public void setOnUpClickListner(OnUpClickListner onUpClickListner){
        this.onUpClickListner = onUpClickListner;
    }
    public interface OnUpClickListner{
        void OnUpClick(int position);
        void OnBackUp(int position);
    }
...
//            }
            ...
            holder.btUpEquip.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    onUpClickListner.OnUpClick(position);
                }
            });
        } else {
            ...
            holder.ivReback.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onUpClickListner.OnBackUp(position);
                }
            });
            if (mList.get(position).getPartsName() != null)
