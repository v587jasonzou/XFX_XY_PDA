...
import jx.yunda.com.terminal.modules.ORGBookNew.adapter.DispatchFwhListAdapterNew;

import jx.yunda.com.terminal.modules.ORGBookNew.presenter.AssignDispatchPresenterNew;
import jx.yunda.com.terminal.modules.ORGBookNew.presenter.IAssignDispatchNew;
...
//范围活fragment
public class AssingFragmentNew extends BaseFragment<AssignDispatchPresenterNew> implements IAssignDispatchNew {
    ...
    ORGBookActivityNew mainAc;
    public static List<FwFormenBean> fwhList = new ArrayList<>();
    DispatchFwhListAdapterNew adapter ;
    int EquipPosition = 0;
    int tabStatus = 0;
    ...
    int userPosition;
    @Override
    protected AssignDispatchPresenterNew getPresenter() {
        return new AssignDispatchPresenterNew(this);
    }
    ...
    public void initData() {
        mainAc = (ORGBookActivityNew) getActivity();
        mContext = getContext();
        mainAc.getProgressDialog().show();
        adapter = new DispatchFwhListAdapterNew(getContext(),fwhList);
        ...
        onTabSelectedListener = new VerticalTabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabView tab, int position) {
                ...
                if(ticketTrainBeans.size()>0){
                    mainAc.getProgressDialog().show();
                    if(tabStatus==0){
                        mPresenter.getFwhList(start,1000,ticketTrainBeans.get(position).getIdx(),0);
                    }else {
                        mPresenter.getCompleteFwhList(start,1000,ticketTrainBeans.get(position).getIdx(),0);
                    }
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                if(ticketTrainBeans.size()>0){
                    ...
                    if(tab.getPosition()==0){
                        tabStatus = 0;
                        adapter.setStatus(0);
                        mPresenter.getFwhList(start,1000,ticketTrainBeans.get(EquipPosition).getIdx(),0);
                    } else {
                        tabStatus = 1;
                        adapter.setStatus(1);
                        mPresenter.getCompleteFwhList(start,1000,ticketTrainBeans.get(EquipPosition).getIdx(),0);
                    }
                }
...
//                            users.addAll(userlistTemp);
    ...
    public void OnLoadTrainSuccess(List<ScheduleTrainBean> list) {
        if(list!=null&&list.size()>0){
            ...
            mPresenter.getFwhList(start,1000,ticketTrainBeans.get(0).getIdx(),0);
        }
        tabTrain.setTabAdapter(new TabAdapter() {
            @Override
            public int getCount() {
