...
public class UpEquipActivity extends BaseActivity<UpEquipPresenter> implements IUpEquip, BaseActivity.OnScanCodeCallBack, EquipListAdapter.OnUpClickListner {
    ...
    int Status = 0;
    int dialogStatus = 0;
    String workPlanId, nodeCaseIDX, trainTypeIDX, ShortName, TrainNo;
    PopupMenu popupMenu, popupMenu2;
    List<ModulesBean> modulesBeans = new ArrayList<>();
    List<String> codes = new ArrayList<>();

    @Override
    protected UpEquipPresenter getPresenter() {
        return new UpEquipPresenter(this);
    }
    ...
    public void initData() {
        ...
        setData();
        adapter = new EquipListAdapter(this, list, "上配件");
...
//                        map.put("identificationCode", etCode.getText().toString());
                    ...
                    getProgressDialog().show();
                    mPresenter.UpEquip(JSON.toJSONString(map),"2");
                    dialog.dismiss();
                }
            });
    ...
    private void setData() {
        ...
        getProgressDialog().show();
        if (Status == 0) {
            mPresenter.getUpEquips(workPlanId, nodeCaseIDX, "1");
        } else {
            mPresenter.getUpEquips(workPlanId, nodeCaseIDX, "2");
        }
    }
    ...
    @Override
    public void getUpEquipSuccess(ArrayList<EquipBean> list) {
        getProgressDialog().dismiss();
...
//            map.put("aboardPlace",list.get(EquipPosition).getUnloadPlace());
                            ...
                            temp.add(map);
                            getProgressDialog().show();
                            mPresenter.UpEquip(JSON.toJSONString(map),"1");
                        }
                    }).show();
            etSearch.setText(code);
        } else {
            ToastUtil.toastShort("请关闭当前页面进行扫码");
        }
