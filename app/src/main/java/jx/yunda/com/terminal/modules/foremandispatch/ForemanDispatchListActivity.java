...
import jx.yunda.com.terminal.modules.foremandispatch.adapter.ForemanDispatchFwhListAdapter;
import jx.yunda.com.terminal.modules.foremandispatch.adapter.ForemanDispatchTicketListAdapter;
import jx.yunda.com.terminal.modules.foremandispatch.model.FwFormenBean;
import jx.yunda.com.terminal.modules.foremandispatch.presenter.ForemanDispatchListPresenter;
...
public class ForemanDispatchListActivity extends BaseActivity<ForemanDispatchListPresenter>
        implements OnViewClickInItemListner, IForemanDispatchList {

    private static int ACT_REQUEST_CODE_FWH = 3001;
    private static int ACT_REQUEST_CODE_TICKET = 3002;
    ...
    ForemanDispatchTicketListAdapter ticketAdapter;
    ForemanDispatchFwhListAdapter fwhAdapter;
    ...
    @Override
    protected ForemanDispatchListPresenter getPresenter() {
        return new ForemanDispatchListPresenter(this);
    }
    ...
    public int getContentViewId() {
        return R.layout.activity_foremandispatch_list;
    }
    ...
    public void initSubViews(View view) {
        tvDispatchHis.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                ActivityUtil.startActivity(ForemanDispatchListActivity.this, ForemanDispatchHisActivity.class, bundle);
            }
        });
    }
    ...
    public void initData() {
        ...
        ticketAdapter = new ForemanDispatchTicketListAdapter(this, mList);
        ticketAdapter.setOnViewClickListner(this);

        fwhAdapter = new ForemanDispatchFwhListAdapter(this, fwhList);
    ...
    private void refreshListData(boolean refresh) {
        ...
        if (0 == tabPosition) {
            ...
            mPresenter.getTicketList(start, IntConstants.PAGE_SIZE,
                    SysInfo.userInfo.emp.getOperatorid().toString(), "false", JSON.toJSONString(param), type);
    ...
    private void firstLoad() {
        ...
        lvTicketList.setAdapter(fwhAdapter);
        mPresenter.getFwhList(start, IntConstants.PAGE_SIZE, workPlanIdx, 0);
    ...
    private void dispatchDirect(int position) {
        if (0 == tabPosition) {
            final FwFormenBean bean = fwhList.get(position);
            if (!TextUtils.isEmpty(bean.getWorkerNameStr())) {
                AlertDialogUtil.confirm(this, "是否直接进行派工？", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        mPresenter.dispatchFwh(bean.getIdx(),
                                bean.getWorkerIdxStr(),
                                bean.getWorkerNameStr());
                    }
                });
            } else {
                goToDispatch(position);
                ...
                AlertDialogUtil.confirm(this, "是否直接进行派工？", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        mPresenter.dispatchTicket(JSON.toJSONString(new String[]{bean.getIdx()}),
                                JSON.toJSONString(new String[]{bean.getDispatchEmpID()}));
                    }
                });
            } else {
                goToDispatch(position);
