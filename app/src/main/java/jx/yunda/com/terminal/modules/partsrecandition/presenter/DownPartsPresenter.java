...
public class DownPartsPresenter extends BasePresenter<IDownParts> {
    public DownPartsPresenter(IDownParts view) {
        super(view);
    }
    public void getDownEquips(String workPlanId , String nodeCaseIDX , String status){
        RequestFactory.getInstance().createApi(TicketApi.class).getDownParts(workPlanId,nodeCaseIDX,status).enqueue(new Callback<BaseBean<ArrayList<ChildEquipBean>>>() {
            @Override
    ...
    public void DownEquip(String str,String str2){
        RequestFactory.getInstance().createApi(TicketApi.class).DownParts(str,str2).enqueue(new Callback<BaseBean>() {
            @Override
